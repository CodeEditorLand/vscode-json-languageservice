...
import { createScanner, JSONScanner, SyntaxKind } from "jsonc-parser";
...
import { Container, PropertyTree } from "./propertyTree";

export function sort(
	documentToSort: TextDocument,
	formattingOptions: SortOptions,
): TextEdit[] {
...
function findJsoncPropertyTree(formattedDocument: TextDocument) {
	...
	let secondToLastNonTriviaNonCommentToken: SyntaxKind | undefined =
		undefined;
	...
	while ((token = scanner.scan()) !== SyntaxKind.EOF) {
		...
		if (
			...
			if (
				secondToLastNonTriviaNonCommentToken ===
		...
		switch (token) {
			...
			case SyntaxKind.StringLiteral: {
				if (
					lastNonTriviaNonCommentToken === undefined ||
					lastNonTriviaNonCommentToken ===
						SyntaxKind.OpenBraceToken ||
					(lastNonTriviaNonCommentToken === SyntaxKind.CommaToken &&
						currentContainerStack[
							currentContainerStack.length - 1
						] === Container.Object)
				) {
					// In that case create the child property which starts at beginningLineNumber, add it to the current tree
					const childProperty: PropertyTree = new PropertyTree(
						scanner.getTokenValue(),
						beginningLineNumber,
					);
					lastProperty = currentProperty;
					currentProperty =
						currentTree!.addChildProperty(childProperty);
				}
				break;
			}
...
function sortJsoncDocument(
	...
	const sortedJsonDocument = TextDocument.create(
		"test://test.json",
		"json",
		0,
	...
	while (queueToSort.length > 0) {
		...
		for (let i = 0; i < propertyTreeArray.length; i++) {
			...
			if (
				propertyTree.lastProperty === true &&
				...
				propertyTree.lastProperty === false &&
				i === propertyTreeArray.length - 1
			) {
...
function updateSortingQueue(
	...
	if (propertyTree.type === Container.Object) {
		...
		const diff =
			minimumBeginningLineNumber - propertyTree.beginningLineNumber!;
		...
		queue.push(
			new SortingRange(
				beginningLineNumber,
				propertyTree.childrenProperties,
			),
		);
...
function updateSortingQueueForArrayProperties(
	...
	for (const subObject of propertyTree.childrenProperties) {
		...
		if (subObject.type === Container.Object) {
			...
			const diff =
				minimumBeginningLineNumber - subObject.beginningLineNumber!;
			queue.push(
				new SortingRange(
					beginningLineNumber +
