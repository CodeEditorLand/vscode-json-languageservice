...
export class JSONCompletion {
					...
					Object.keys(schemaProperties).forEach((key: string) => {
						...
						if (
							...
							if (propertySchema.completionDetail !== undefined) {
								proposal.detail =
									propertySchema.completionDetail;
							}
					...
					const propertyNameCompletionItem = (
						...
						const proposal: JSONCompletionItem = {
							...
							documentation:
								enumDescription ||
								this.fromMarkup(
									schemaPropertyNames.markdownDescription,
								) ||
								schemaPropertyNames.description ||
								"",
						};
						if (
							schemaPropertyNames.completionDetail !== undefined
						) {
							proposal.detail =
								schemaPropertyNames.completionDetail;
						}
	...
	private getContributedValueCompletions(
		...
		document: TextDocument,
		collector: CompletionsCollector,
		collectionPromises: PromiseLike<any>[],
	) {
		if (!node) {
			this.contributions.forEach((contribution) => {
